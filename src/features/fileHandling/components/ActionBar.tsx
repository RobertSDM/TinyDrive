...
import useDeleteFolderById, {
    useDownloadFile,
    useDownloadFolder,
    useUpdateName,
} from "../hooks/fileHandlingHooks.tsx";
...
export default function ActionBar({ parentFolderId }: ActionBarProps) {
    ...
    const { request: delete_ } = useDeleteFolderById();
    const { request: update } = useUpdateName(selectedItem!);
    const { close: closeModal, open: openModal, isOpen } = useModalContext();
